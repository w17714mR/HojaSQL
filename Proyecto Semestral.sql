   ...
   WHERE
      ...
      IN ((
            ...
            FROM
               GASTO_COMUN
            WHERE
               PCO_PERIODO = 201907) MINUS ( --EXTRACT (YEAR FROM SYSDATE)||(EXTRACT(MONTH FROM SYSDATE)-1))
               SELECT
                  (NRO_DEPARTAMENTO)
               FROM
                  PAGO_GASTO_COMUN
...
WHERE
   ...
   AND EXTRACT(MONTH FROM GAS_FECHA_PAGO) = 07 -- EXTRACT ()(MONTH FROM SYSDATE)-1)
   AND EXTRACT(YEAR FROM SYSDATE) = 2019 -- EXTRACT (YEAR FROM SYSDATE)
ORDER BY
   7 DESC;
...
WHERE
   EXTRACT(MONTH FROM GAS_FECHA_PAGO) = 08 -- (EXTRACT (MONTH FROM SYSDATE)-1)
   AND EXTRACT(YEAR FROM GAS_FECHA_PAGO) = 2019 -- EXTRACT (YEAR FROM SYSDATE)
ORDER BY
   1 ASC,
   2 ASC;
...
AND SUBSTR(GASTO_COMUN.PCO_PERIODO, 1, 4) = EXTRACT(YEAR FROM SYSDATE)
AND SUBSTR(GASTO_COMUN.PCO_PERIODO, 5, 2) < 08 -- EXTRACT(MONTH FROM SYSDATE)-1
ORDER BY
   3 ASC,
   1 ASC,
   5 DESC;
...
FROM
   ...
   JOIN EDIFICIO USING (ID_EDIFICIO)
WHERE
   EXTRACT(MONTH FROM GAS_FECHA_PAGO) = 08
   AND EXTRACT(YEAR FROM GAS_FECHA_PAGO) = 2019
ORDER BY
   1 ASC,
   2 ASC;
...
WHERE
   SUBSTR(G.PCO_PERIODO, 1, 4) = EXTRACT(YEAR FROM SYSDATE)
   AND SUBSTR(G.PCO_PERIODO, 5, 2) < EXTRACT(MONTH FROM SYSDATE) - 3
GROUP BY
   G.NRO_DEPARTAMENTO,
